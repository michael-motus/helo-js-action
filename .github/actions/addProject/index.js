...
async function action (){
  ...
  const stages = githubApi.motusDefault.stages;
  ...
  try {
    ...
    const resultProject = await githubApi.addRepoProject(repoConfig, projectName, projectBody);
    console.log("project result: ", JSON.stringify(resultProject));
    var projID = resultProject.id;
    console.log(projID);
    for(const stage of stages) {
        const resultColumn = await githubApi.addProjectColumn(repoConfig, resultProject.id, stage.name)
